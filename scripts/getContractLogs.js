...
async function main() {
	...
	// get contract id from .env or from first argument
	const contractIdString = process.env.CONTRACT_ID ?? process.argv[2];

	if (contractIdString === undefined || contractIdString == null) {
		console.log('Contract ID required, please specify CONTRACT_ID in the .env file or as the first argument');
		return;
	}
	else if (contractIdString.match(/[0-9]+\.[0-9]+\.[0-9]+/) == null) {
		console.log('Contract ID should be Hedera native format 0.0.XXXX, please specify CONTRACT_ID in the .env file or as the first argument');
		return;
	}
